...
if [ -z $month_number ]
then
  echo	"No month number given! Please, enter a month number in the command line"
	echo	"Eg month 4 = April"
	exit
fi

if [[ $month_number -lt 1 ]] || [[ $month_number -gt 12 ]]
then
	echo	"Invalid month number given, Please, enter month number from 1 - 12"
	exit
fi
